...
util::Status FrameSync::create(const CreateInfo &ci) {
  ...
  alloc_ = ci.allocator;
  fence_signaled_on_create_ = ci.fence_signaled_on_create;

  return create_per_frame_(ci.frames_in_flight);
}
...
void FrameSync::destroy() noexcept {
  ...
  alloc_ = nullptr;
  fence_signaled_on_create_ = true;
}
...
util::Status FrameSync::create_per_frame_(uint32_t count) {
  ...
  fence_info.flags =
      fence_signaled_on_create_ ? VK_FENCE_CREATE_SIGNALED_BIT : 0;
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    QUARK_VK_TRY(
        vkCreateFence(vk_device_, &fence_info, alloc_, &frames_[i].in_flight));
  }
...
void FrameSync::destroy_per_frame_() noexcept {
  ...
  for (auto &f : frames_) {
    if (f.image_available != VK_NULL_HANDLE) {
      vkDestroySemaphore(vk_device_, f.image_available, alloc_);
      f.image_available = VK_NULL_HANDLE;
    }

    if (f.render_finished != VK_NULL_HANDLE) {
      vkDestroySemaphore(vk_device_, f.render_finished, alloc_);
      f.render_finished = VK_NULL_HANDLE;
    }

    if (f.in_flight != VK_NULL_HANDLE) {
      vkDestroyFence(vk_device_, f.in_flight, alloc_);
    }
  }
